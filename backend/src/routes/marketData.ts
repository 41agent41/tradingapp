      ...
      try {
        ...
        if (includeIndicators && response.data?.indicators) {
          for (const bar of response.data.data) {
            ...
            if (bar.macd_signal)
              indicators.push({ name: 'MACD_SIGNAL', period: 26, value: bar.macd_signal });
            if (bar.bb_upper)
              indicators.push({ name: 'BB_UPPER', period: 20, value: bar.bb_upper });
            if (bar.bb_middle)
              indicators.push({ name: 'BB_MIDDLE', period: 20, value: bar.bb_middle });
            if (bar.bb_lower)
              indicators.push({ name: 'BB_LOWER', period: 20, value: bar.bb_lower });

            if (indicators.length > 0) {
              await marketDataService.storeTechnicalIndicators(
                contractId,
                timeframe,
                timestamp,
                indicators
  ...
  try {
    ...
    if (useDatabase) {
      try {
        ...
        if (dbData.length > 0) {
          console.log(
            `Retrieved ${dbData.length} bars with indicators from database for ${symbol} ${timeframe}`
          );
