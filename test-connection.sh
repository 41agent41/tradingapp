...
# Test backend root
echo "📡 Testing Backend Root (localhost:4000/)..."
...
# Test backend health
echo "📡 Testing Backend Health (localhost:4000/health)..."
...
echo "📡 Testing Settings API (localhost:4000/api/settings)..."
if curl -s http://localhost:4000/api/settings > /dev/null; then
    echo "✅ Settings API is responding"
    curl -s http://localhost:4000/api/settings | jq . 2>/dev/null || curl -s http://localhost:4000/api/settings
else
...
# Test IB service
echo "📡 Testing IB Service (localhost:8000/)..."
...
echo "📡 Testing IB Service Health (localhost:8000/health)..."
...
echo "📡 Testing IB Gateway Health (localhost:8000/gateway-health)..."
...
echo "📡 Testing IB Connection Status (localhost:8000/connection)..."
if curl -s http://localhost:8000/connection > /dev/null; then
    echo "✅ IB Connection status is responding"
    curl -s http://localhost:8000/connection | jq . 2>/dev/null || curl -s http://localhost:8000/connection
else
